...
class CampaignProcessingService {
  ...
  async processCampaign(campaign) {
    try {
      ...
      // Check the sending number's health before committing to a batch send.
      // A RED quality rating or an exhausted messaging tier means this batch
      // is likely to fail/get throttled by Meta anyway - better to hold the
      // campaign here than burn through the audience list with failed sends.
      const healthCheck = await this.checkPhoneNumberHealth(campaign.organization_id);
      if (healthCheck && !healthCheck.healthy) {
        logger.warn("Holding campaign - phone number health check failed", {
          campaignId: campaign.id,
          organizationId: campaign.organization_id,
          reason: healthCheck.reason,
        });
        // Leave status as 'asset_generated' so it's picked up again next
        // cycle once quality/tier recovers, instead of silently dropping it.
        return;
      }
  ...
  /**
   * Check the organization's WhatsApp phone number quality/tier before a
   * campaign batch send. Best-effort: if the health check itself fails
   * (network issue, credentials not configured yet, etc) we don't block the
   * campaign on that - only an explicit RED/unhealthy result blocks sending.
   * @param {string} organizationId
   * @returns {Promise<{healthy: boolean, reason?: string, data?: object}|null>}
   */
  async checkPhoneNumberHealth(organizationId) {
    try {
      ...
      if (
        ...
        // No credentials configured - let the send attempt itself surface
        // that error rather than blocking here.
        return null;
      }
      ...
      if (health?.quality_rating === "RED") {
        return {
          healthy: false,
          reason: `Phone number quality rating is RED (${health.display_phone_number || organizationId})`,
          data: health,
        };
      }
      ...
      if (health?.status && health.status !== "CONNECTED") {
        return {
          healthy: false,
          reason: `Phone number status is "${health.status}", expected CONNECTED`,
          data: health,
        };
      }
      ...
      // Don't let a health-check failure (rate limit, transient network
      // error) block campaigns entirely - log it and let the send proceed.
      logger.warn("Phone number health check failed, proceeding without it", {
        organizationId,
        error: error.message,
      });
      return null;
