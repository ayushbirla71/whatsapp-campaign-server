...
const sampleTemplate = {
  ...
  language: "en",
  body_text:
    "Hi {{1}}, welcome to our store! Get {{2}}% off your first purchase.",
  components: [
    {
      type: "HEADER",
      format: "TEXT",
...
async function testSQSIntegration() {
  ...
  try {
    ...
    if (!isConfigured) {
      console.log(
        "⚠️  SQS is not configured. Please set up AWS credentials and queue URL."
      );
      return;
    }
...
async function testMediaMessage() {
  ...
  const mediaTemplate = {
    ...
    body_text:
      "Special offer for {{name}}! Check out this amazing deal with caption support.",
  };
