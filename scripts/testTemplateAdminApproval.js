...
async function testTemplateAdminApproval() {
  ...
  try {
    ...
    if (approvedTemplate.status === 'approved' && approvedTemplate.approved_by_admin === 'approved') {
      console.log('✓ Campaign creation validation passed - template is admin approved');
    } else {
    ...
    console.log('Generated message payload:', JSON.stringify(messagePayload, null, 2));
    ...
    if (templateParams && templateParams.length > 0) {
      ...
      templateParams.forEach((param, index) => {
        console.log(`  Parameter ${index + 1}: ${param.value} (type: ${param.type})`);
      });
    ...
    const legacyBodyParams = legacyMessagePayload.templateParameters?.filter(p => p.type === 'body') || [];
    if (legacyBodyParams.length > 0) {
      console.log('✓ Legacy parameter fallback working');
      console.log('Legacy values:', legacyBodyParams.map(p => p.value));
    }
    ...
    console.log('✓ Extracted parameters:', extractedParams.map(p => p.value));
    
    // Test 7: Test placeholder replacement
    console.log('\nTest 7: Testing placeholder replacement...');
    
    const testText = 'Hello {{1}}, your order {{2}} is ready for pickup at {{3}}!';
    ...
    if (replacedText === 'Hello John Doe, your order ORD-12345 is ready for pickup at Downtown Store!') {
...
function printUsageExamples() {
  ...
  console.log('    {"type": "body", "valueType": "text", "value": "John Doe"},');
  console.log('    {"type": "body", "valueType": "text", "value": "ORD-12345"},');
  console.log('    {"type": "body", "valueType": "text", "value": "Downtown Store"}');
  console.log('  ]');
  console.log('}');
}
