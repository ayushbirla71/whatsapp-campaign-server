...
const updateTemplate = asyncHandler(async (req, res) => {
  ...
  // If this template was already pushed to WhatsApp (has a whatsapp_template_id),
  // push the content change to Meta too, so the two stay in sync. Templates
  // that were never pushed (still local/draft) don't need this.
  let whatsappSyncError = null;
  if (template.whatsapp_template_id && (updateData.components || updateData.category)) {
    const whatsappConfig = await Organization.getWhatsAppConfig(
      template.organization_id
    );

    if (whatsappConfig?.whatsapp_access_token) {
      try {
        await whatsappApiService.updateTemplateOnWhatsApp(
        ...
        logger.info("Template update pushed to WhatsApp API", {
          templateId,
          whatsappTemplateId: template.whatsapp_template_id,
        });
      } catch (pushError) {
        whatsappSyncError = pushError.message;
        logger.error("Failed to push template update to WhatsApp API", {
          templateId,
  ...
  res.json({
    success: true,
    message: whatsappSyncError
      ? `Template updated locally, but WhatsApp sync failed: ${whatsappSyncError}`
      : "Template updated successfully",
    data: {
      template: Template.parseTemplate(updatedTemplate),
      whatsapp_sync_error: whatsappSyncError,
    },
  });
});
...
const deleteTemplate = asyncHandler(async (req, res) => {
  ...
  // If this template exists on WhatsApp, delete it there too - otherwise
  // it stays live on Meta's side forever even though it's gone from our DB.
  let whatsappSyncError = null;
  if (template.whatsapp_template_id) {
    ...
    if (whatsappConfig?.whatsapp_business_account_id && whatsappConfig?.whatsapp_access_token) {
      try {
        await whatsappApiService.deleteTemplateOnWhatsApp(
          whatsappConfig.whatsapp_business_account_id,
          template.name,
          whatsappConfig.whatsapp_access_token
        );
        logger.info("Template deleted from WhatsApp API", {
          templateId,
          whatsappTemplateId: template.whatsapp_template_id,
        });
      } catch (deleteError) {
        whatsappSyncError = deleteError.message;
        logger.error("Failed to delete template from WhatsApp API", {
          templateId,
  ...
  res.json({
    success: true,
    message: whatsappSyncError
      ? `Template deleted locally, but WhatsApp deletion failed: ${whatsappSyncError}. It may still be live on WhatsApp.`
      : "Template deleted successfully",
    data: { whatsapp_sync_error: whatsappSyncError },
  });
});
...
const approveTemplate = asyncHandler(async (req, res) => {
  ...
  // Internal (staff) approval is our own quality gate before we spend a
  // real submission against Meta's template review quota. Once staff
  // approves, push the template to WhatsApp for actual review - Meta's
  // real decision (APPROVED/REJECTED/DISABLED) comes back later via the
  // `message_template_status_update` webhook, which updates whatsapp_status.
  let whatsappPushResult = null;
  let whatsappPushError = null;
  ...
  if (
    ...
    try {
      ...
      whatsappPushResult = await whatsappApiService.createTemplateOnWhatsApp(
        whatsappConfig.whatsapp_business_account_id,
        {
          name: template.name,
      ...
      logger.info("Template pushed to WhatsApp API after internal approval", {
        templateId,
        whatsappTemplateId: whatsappPushResult?.id,
        whatsappStatus: whatsappPushResult?.status,
      });
    } catch (pushError) {
      // Don't block the internal approval on a Meta-side failure (duplicate
      // name, malformed components, rate limit, etc) - surface it instead so
      // the admin can see the template is approved internally but NOT yet
      // live on WhatsApp, and can retry the push.
      whatsappPushError = pushError.message;
      logger.error("Failed to push approved template to WhatsApp API", {
        templateId,
        error: pushError.message,
      });
    }
    ...
    whatsappPushError =
      "Organization has no WhatsApp Business API credentials configured - template approved internally only";
    logger.warn(whatsappPushError, { templateId, organizationId: template.organization_id });
  }
  ...
  // Record whatever Meta gave us back (usually whatsapp_status: "PENDING")
  let finalTemplate = updatedTemplate;
  if (whatsappPushResult?.id) {
    finalTemplate = await Template.update(templateId, {
      whatsapp_template_id: whatsappPushResult.id,
      whatsapp_status: whatsappPushResult.status || "PENDING",
    });
  }
  ...
  logger.info("Template approved", {
    ...
    templateName: template.name,
    approvedBy: req.user.id,
    pushedToWhatsApp: !!whatsappPushResult,
  });

  res.json({
    success: true,
    message: whatsappPushError
      ? `Template approved internally, but was not pushed to WhatsApp: ${whatsappPushError}`
      : "Template approved and submitted to WhatsApp for review",
    data: {
      template: Template.parseTemplate(finalTemplate),
      whatsapp_sync_error: whatsappPushError,
    },
  });
});
