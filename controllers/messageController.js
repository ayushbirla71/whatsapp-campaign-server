...
exports.getConversationMessages = async (req, res) => {
  try {
    ...
    // Verify the requesting user's organization actually owns this
    // conversation before returning its message history.
    const conversation = await Conversation.findById(conversationId);
    if (!conversation) {
      return res.status(404).json({
        success: false,
...
exports.markConversationRead = async (req, res) => {
  try {
    ...
    // Tell WhatsApp the customer's latest inbound message has been read, so
    // the customer sees the blue double-check. Best-effort - a failure here
    // shouldn't block the agent from clearing their own unread badge.
    let whatsappReadError = null;
    try {
      const { rows: lastInboundRows } = await pool.query(
        `
      ...
      if (lastInboundRows.length > 0) {
        ...
        if (whatsappConfig?.whatsapp_phone_number_id && whatsappConfig?.whatsapp_access_token) {
          await whatsappApiService.markMessageAsRead(
            whatsappConfig.whatsapp_phone_number_id,
            lastInboundRows[0].whatsapp_message_id,
            whatsappConfig.whatsapp_access_token
          );
        }
      }
    } catch (readError) {
      whatsappReadError = readError.message;
      console.error("Failed to mark message as read on WhatsApp API:", readError.message);
    }
    ...
    res.json({
      success: true,
      whatsapp_read_error: whatsappReadError,
    });
  } catch (error) {
    console.error("markConversationRead error:", error);
    res.status(500).json({
